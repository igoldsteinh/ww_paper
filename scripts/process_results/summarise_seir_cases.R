...
full_stan_diag <- map(seed_vals, ~read_csv(paste0("results/seir_cases/mcmc_summaries/mcmc_summary_scenario", 
                                                             snum, 
                                                             "_seed",
                                                             .x,
                                                             ".csv"))) %>%
...
write_csv(full_stan_diag, here::here("results", "seir_cases", paste0("seir_cases_scenario",snum, "_allseeds_stan_diag.csv")))
...
timevarying_quantiles <- map(seed_vals, ~read_csv(paste0("results/seir_cases/generated_quantities/posterior_timevarying_quantiles_scenario",
                                                                    snum,
                                                                    "_seed",
                                                                    .x,
                                                                    ".csv"))) 
...
write_csv(rt_quantiles, here::here("results", "seir_cases", paste0("seir_cases_scenario", snum,  "_allseeds_rt_quantiles.csv")))
write_csv(rt_quantiles, here::here("results", "seir_cases", paste0("seir_cases_scenario", snum,  "_allseeds_r0_quantiles.csv")))
write_csv(I_quantiles, here::here("results", "seir_cases", paste0("seir_cases", snum,  "_allseeds_prevI_quantiles.csv")))
...
make_rt_plot <- function(seed_val) {
  rt_quantiles %>%
    ...
    ggtitle(str_c("SEIR Cases Posterior Rt Scenario ", snum, " Seed ", seed_val)) +
    my_theme
}
...
ggsave2(filename = here::here("results", "seir_cases", paste0("seir_cases_rt_plots_scenario", snum, ".pdf")),
        plot = rt_quantiles %>%
          distinct(seed) %>%
          arrange(seed) %>%
          pull(seed) %>%
...
ggsave2(filename = here::here("results", "seir_cases", paste0("seir_cases_r0_plots_scenario", snum, ".pdf")),
        plot = r0_quantiles %>%
          distinct(seed) %>%
          arrange(seed) %>%
          pull(seed) %>%
