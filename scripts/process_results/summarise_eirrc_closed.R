...
num_sims = 100
seed = 1:num_sims
full_stan_diag <- map(seed, ~read_csv(paste0("results/eirrc_closed/mcmc_summaries/mcmc_summary_scenario", 
                                                             sim, 
                                                             "_seed",
                                                             .x,
                                                             ".csv"))) %>%
...
write_csv(full_stan_diag, here::here("results", "eirrc_closed", paste0("eirr_scenario", sim,  "_allseeds_stan_diag.csv")))
...
timevarying_quantiles <- map(seed, ~read_csv(paste0("results/eirrc_closed/generated_quantities/posterior_timevarying_quantiles_scenario",
                                                                    sim,
                                                                    "_seed",
                                                                    .x,
                                                                    ".csv"))) 
...
write_csv(rt_quantiles, here::here("results", "eirrc_closed", paste0("eirr_scenario", sim,  "_allseeds_rt_quantiles.csv")))
...
ggsave2(filename = here::here("results", "eirrc_closed", paste0("eirr_rt_plots_scenario", sim, ".pdf")),
        plot = rt_quantiles %>%
          distinct(seed) %>%
          arrange(seed) %>%
          pull(seed) %>%
