#bayesian alpha model eirr using ODE solver to compare computation times
using LinearAlgebra
...
@model function bayes_eirr_student(data_log_copies, obstimes, param_change_times, extra_ode_precision, prob, abs_tol, rel_tol, savetimes, index)
  ...
  ν_non_centered ~ Normal() # rate to Re
  η_non_centered ~ Normal() # rate to Rd
  ρ_gene_non_centered ~ Normal() # gene detection rate
